...
bool CHttpAsync::Post(std::string   url,
  ...
  resp_data_ = "";
  req_data_.append(post_data, post_data_size);

  // ³¬Ê±
  int resp_timeout = HTTP_TIMEOUT_RESP;
  ...
  curl_easy_setopt(ceasy_, CURLOPT_POSTFIELDS,    req_data_.c_str());
  curl_easy_setopt(ceasy_, CURLOPT_POSTFIELDSIZE, req_data_.size());
