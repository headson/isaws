...
HI_S32 SAMPLE_COMM_SYS_MemConfig(HI_VOID) {
  ...
  /*the max chn number of vi,vdec is 64*/
  for(i=0; i<32; i++) {
...
/******************************************************************************
* function : vb init with DCF & MPI system init
******************************************************************************/
HI_S32 SAMPLE_COMM_SYS_Init_With_DCF(VB_CONF_S* pstVbConf) {
  VB_SUPPLEMENT_CONF_S stSupplementConf = {0};
  MPP_SYS_CONF_S stSysConf = {0};
  HI_S32 s32Ret = HI_FAILURE;
  ...
  stSupplementConf.u32SupplementConf = 1;
