...
void* vpss_chn_dump(void* pArg) {
  VPSS_GRP vGrp = 0;
  VPSS_CHN vChn = 1;
  HI_U32 u32Depth = 2;

  bool bSendToVgs = HI_FALSE;
  ...
  if (HI_MPI_VPSS_GetDepth(vGrp, vChn, &u32OrigDepth) != HI_SUCCESS) {
    printf("get depth error!!!\n");
    return (HI_VOID*)-1;
  }

  if (HI_MPI_VPSS_SetDepth(vGrp, vChn, u32Depth) != HI_SUCCESS) {
    printf("set depth error!!!\n");
    return (HI_VOID*)-1;
  }
  u32VpssDepthFlag = 1;
  ...
  stFrame.u32PoolId = VB_INVALID_POOLID;
  while (HI_MPI_VPSS_GetChnFrame(vGrp, vChn, &stFrame, s32MilliSec) != HI_SUCCESS) {
    s32Times--;
    if (0 >= s32Times) {
