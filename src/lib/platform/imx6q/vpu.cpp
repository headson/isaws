...
extern "C" {
#endif
#include "sdk/imx6q/inc/vpu_jpegtable.h"
#ifdef __cplusplus
}
#endif
...
int32_t CVpu::vpu_setup()
{
    ...
    if (nRet)
    {
        loge("Unable to obtain physical memory.");
    ...
    {
        loge("Encoder open failed %d %d-%d", nRet, nWidth, nHeight);
        return RET_INVALID_HDL;
    }
    enc_fd = handle;

    logd("hdl 0x%x, w %d, h %d, enc %d, bit %d, iGop %d.", 
...
void CVpu::vpu_close()
{
    if (enc_fd)
    {
        logd("hdl 0x%x close.", (uint32_t)enc_fd);
...
int32_t CVpu::vpu_alloc_encbuffs()
{
    ...
    logd("hdl 0x%x frame number %d.", (uint32_t)enc_fd, nFrmNums);
    // DST frame buffer
    pFrmDesc = (vpu_mem_desc *)calloc(sizeof(vpu_mem_desc), nFrmNums);
    if (pFrmDesc == NULL) {
        loge("hdl 0x%x Can't calloc vpu_mem_desc.", (uint32_t)enc_fd);
        goto err;
    }

    pFrmBufs = (FrameBuffer*)calloc(sizeof(FrameBuffer), nFrmNums);
    if (pFrmBufs == NULL) {
        loge("hdl 0x%x Can't calloc FrameBuffer.", (uint32_t)enc_fd);
        goto err;
    }
    ...
    for (i = 0; i < initinfo.minFrameBufferCount+nExtFrmBuffCount; i++)
    {
        ...
        {
            loge("hdl 0x%x Frame buffer allocation failure.", (uint32_t)enc_fd);
            goto err;
        }
    ...
    if (nRet != RETCODE_SUCCESS)
    {
        loge("hdl 0x%x Register frame buffer failed", (uint32_t)enc_fd);
        goto err;
    }
    return RET_SUCCESS;
...
err:
    vpu_free_encbuffs();
    loge("hdl 0x%x vpu_alloc_encbuffs failed.", (uint32_t)enc_fd);
    return RET_MEMORY_LEAK;
}
...
void CVpu::vpu_free_encbuffs()
{
    if (pFrmDesc)
    {
        ...
        free(pFrmDesc); pFrmDesc = NULL;
    }
    
    if (pFrmBufs) { free(pFrmBufs); pFrmBufs = NULL; }
}
