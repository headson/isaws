...
LINKED_LIST* h_list_create (BOOL bNeedMutex)
{
	...
	p_linked_list = (LINKED_LIST*)get_one_item(ONVIF_TYPE_LINK_LIST);// malloc (sizeof (LINKED_LIST));
...
void h_list_free_container (LINKED_LIST* p_linked_list)
{
	...
	while (p_node != NULL) 
	{
		...
		if(p_free != NULL)
		  free_one_item(p_free,ONVIF_TYPE_NOTIFYMESSAGE);
			//free(p_free);
		free_one_item(p_node,ONVIF_TYPE_LINK_NODE);
		//free (p_node);
  ...
  free_one_item(p_linked_list,ONVIF_TYPE_LINK_LIST);
	//free (p_linked_list);
}
...
void h_list_free_all_node (LINKED_LIST* p_linked_list)
{
	...
	while (p_node != NULL) 
	{
      ...
      free_one_item(p_node->p_data,ONVIF_TYPE_NOTIFYMESSAGE);
			//free(p_node->p_data);
    free_one_item(p_node,ONVIF_TYPE_LINK_NODE);
		//free (p_node);
...
BOOL h_list_add_at_front (LINKED_LIST* p_linked_list, void* p_item)
{
	...
	p_node = (LINKED_NODE*)get_one_item(ONVIF_TYPE_LINK_NODE);// malloc (sizeof (LINKED_NODE));
...
void h_list_remove_from_front (LINKED_LIST* p_linked_list)
{
  ...
  free_one_item(p_node_to_remove,ONVIF_TYPE_LINK_NODE);
...
BOOL h_list_insert(LINKED_LIST * p_linked_list,LINKED_NODE * p_pre_node,void *p_item)
{
	...
	else
	{
		...
		{
			LINKED_NODE * p_node = (LINKED_NODE *)get_one_item(ONVIF_TYPE_LINK_NODE);//malloc(sizeof(LINKED_NODE));
			if (NULL == p_node)
			{
			    return FALSE;
			}
