...
{
	if (rx_msg->ctt_type == CTT_XML)
	{
		soap_process_request(p_user, rx_msg);	
	}
	else if (rx_msg->ctt_type == CTT_BIN)
	{
	...
	else if (rx_msg->ctt_type == CTT_NULL)
	{
	    char * post = rx_msg->first_line.value_string;
		if (strstr(post, "snapshot"))	        
		{
...
BOOL http_tcp_rx(HTTPCLN * p_user)
{
  ...
  //printf("http_tcp_rx rcv_dlen:%d,(max %d)\n",p_user->rcv_dlen,BUFFER_SIZE);
  if (p_user->rcv_dlen  >= BUFFER_SIZE-1) {
		log_print("http_tcp_rx::recv dlen[%d], mlen[%d]\r\n",p_user->rcv_dlen,p_user->mlen);
		closesocket(p_user->cfd);
		p_user->cfd = 0;
		return FALSE;
  } 
...
rx_analyse_point:
	...
	if(p_user->hdr_len == 0)
	{
		...
		if (ctt_is_stirng(rx_msg->ctt_type))
			log_print("RX >> %s\r\n\r\n",rx_msg->msg_buf);
	...
	{
		log_print("http_tcp_rx::user input data is too large ctt_len[%d], hdr_len[%d], mlen[%d]\r\n",p_user->ctt_len + p_user->hdr_len,p_user->mlen);
		closesocket(p_user->cfd);
		p_user->cfd = 0;
		return FALSE;
    /*
...
int http_srv_net_init(HTTPSRV * p_srv)
{
	...
	addr.sin_addr.s_addr = htonl(INADDR_ANY);//p_srv->saddr;
...
int http_srv_init(HTTPSRV * p_srv, unsigned int saddr, unsigned short sport, int cln_num)
{
	...
	p_srv->saddr = htonl(INADDR_ANY);//saddr;
	p_srv->sport = sport;
