...
int hxml_parse_element_end(LTXMLPRS * parse)
{
	...
	*(parse->ptr) = '\0';
	...
	{
		printf("hxml_parse_element_end::cur name[%s] != stack name[%s]!!!\r\n", end_name, stack_name);
		return -1;
	}
...
int hxml_parse_element_start(LTXMLPRS * parse)
{
	...
	else
	{
		...
		*(parse->ptr) = '\0'; (parse->ptr)++;
		...
		ret = hxml_parse_attr(parse); if(ret < 0) return -1;
...
{
	// printf("hxml parse header start...\r\n");
	...
	// printf("hxml parse element start...\r\n");
