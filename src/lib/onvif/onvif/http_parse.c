...
BOOL http_get_auth_digest_info(HTTPMSG * rx_msg, HD_AUTH_INFO * p_auth)
{
    ...
    HDRV * res_line = find_http_headline(rx_msg, "Authorization");
	if (res_line == NULL)
		return FALSE;
		
	GetLineWord(res_line->value_string,0,strlen(res_line->value_string),
		word_buf,sizeof(word_buf),&next_offset,WORD_TYPE_STRING);
		
	if (stricmp(word_buf,"digest") != 0)
		return FALSE;

	if (GetNameValuePair(res_line->value_string+next_offset,
		strlen(res_line->value_string)-next_offset,"username",p_auth->auth_name,sizeof(p_auth->auth_name)) == FALSE)
		return FALSE;

	if (GetNameValuePair(res_line->value_string+next_offset,
		strlen(res_line->value_string)-next_offset,"realm",p_auth->auth_realm,sizeof(p_auth->auth_realm)) == FALSE)
		return FALSE;

	if (GetNameValuePair(res_line->value_string+next_offset,
		strlen(res_line->value_string)-next_offset,"nonce",p_auth->auth_nonce,sizeof(p_auth->auth_nonce)) == FALSE)
		return FALSE;

	if (GetNameValuePair(res_line->value_string+next_offset,
		strlen(res_line->value_string)-next_offset,"uri",p_auth->auth_uri,sizeof(p_auth->auth_uri)) == FALSE)
		return FALSE;

	if (GetNameValuePair(res_line->value_string+next_offset,
		strlen(res_line->value_string)-next_offset,"response",p_auth->auth_response,sizeof(p_auth->auth_response)) == FALSE)
		return FALSE;

	if (GetNameValuePair(res_line->value_string+next_offset,
		strlen(res_line->value_string)-next_offset,"qop",p_auth->auth_qop,sizeof(p_auth->auth_qop)))
	{
        char * stop_string;
        
		if(GetNameValuePair(res_line->value_string+next_offset,
			strlen(res_line->value_string)-next_offset,"cnonce",p_auth->auth_cnonce,sizeof(p_auth->auth_cnonce)) == FALSE)
			p_auth->auth_cnonce[0] = '\0';

		if(GetNameValuePair(res_line->value_string+next_offset,
			strlen(res_line->value_string)-next_offset,"nc",p_auth->auth_ncstr,sizeof(p_auth->auth_ncstr)) == FALSE)
