...
#define NODE_MAX  128
static LocalXMLNStruct g_nodes[NODE_MAX]={0};

XMLN * get_one_xmln() {
  int i = -1;
  for (i = 0; i < NODE_MAX; i++) {
    if (g_nodes[i].flag == 0) {
      //printf("xml get node[%d] !!!!!!!!\n",i);
      g_nodes[i].flag = 1;
      return &g_nodes[i].node;
    }
  }
...
void free_one_xmln(XMLN *node) {
  unsigned int index = (((unsigned int)node - (unsigned int)&g_nodes[0].node)/(unsigned int)sizeof(LocalXMLNStruct));
  //printf("xml free node[%d] !!!!!!!!\n",index);
  if(index >= NODE_MAX) {
   printf("index[%u] error!!!!!!!!\n",index);
  }
  g_nodes[index].flag = 0;
}
...
{
	XMLN * p_node = get_one_xmln();//(XMLN *)malloc(sizeof(XMLN));
	if (p_node == NULL)
	{
...
XMLN * xml_attr_add(XMLN * p_node, const char * name, const char * value)
{
	...
	p_attr = get_one_xmln();//(XMLN *)malloc(sizeof(XMLN));
	if (p_attr == NULL)
	{
