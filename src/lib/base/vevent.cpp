...
void EVT_LOOP::Process()
{
...
#endif
    logd("%s base event start %d.", get_name().c_str(), VThread::get_pid());

    int32_t nRet = 0;
    while (_isRuning && m_pEvent)
    {
        VThread::msleep(200);
        nRet = event_base_loop(m_pEvent, 0);
        if (nRet < 0) {                 // 表示发生了错误
#ifdef _CLIENT
            loge("%s event loop exit %d, pid %d.\n", get_name().c_str(), nRet, VThread::get_pid());
#endif
            break;
        }
    ...
    logd("%s base event exit %d.", get_name().c_str(), VThread::get_pid());
}
...
void EVT_TIMER::set_name(const string& sName)
{
    m_sName = sName;
}

const string& EVT_TIMER::get_name() const
{
    return m_sName;
}
...
void EVT_IO::set_name(const string& sName)
{
    m_sName = sName;
}

const string& EVT_IO::get_name() const
{
    return m_sName;
}
...
int32_t EVT_IO::Start(VHANDLER vHdl, int32_t nEvt)
{
