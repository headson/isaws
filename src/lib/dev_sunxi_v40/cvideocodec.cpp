...
void ParseArgument(encode_param_t *encode_param, char *argument, char *value)
{
    ...
    switch(arg)
    {
        ...
        case SRC_SIZE:
            ...
            printf(" get src_size: %dp ", encode_param->src_size);
            if(encode_param->src_size == 352)
            {
        ...
        case DST_SIZE:
            ...
            printf(" get dst_size: %dp ", encode_param->dst_size);
            if(encode_param->dst_size == 352)
            {
...
{
    printf(" ==== CedarX2.0 encoder demo help start ===== ");
    printf(" -h or --help to show the demo usage");
    printf(" demo created by yangcaoyuan, allwinnertech/AL3 ");
    printf(" email: yangcaoyuan@allwinnertech.com ");
    printf(" ===== CedarX2.0 encoder demo help end ====== ");
}
...
int main(int argc, char** argv)
{
    ...
    encode_param.bit_rate = 256*1024;
    encode_param.frame_rate = 30;
    encode_param.maxKeyFrame = 30;

    encode_param.encode_format = VENC_CODEC_H264;
    encode_param.encode_frame_num = 500;

    strcpy((char*)encode_param.intput_file,        "./cif.yuv");
    strcpy((char*)encode_param.output_file,        "./out.264");
    strcpy((char*)encode_param.reference_file,     "./test.jpg");
    ...
    while(testNumber < encode_param.encode_frame_num)
    {
        ...
        switch(TemporalLayer)
        {
            ...
            case 2:
            case 1:
                printf("just write the PrefixNAL\n");
