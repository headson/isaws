...
char *put_byte(char *output, uint8 nVal) {
  output[0] = nVal;
  return output+1;
}
char *put_be16(char *output, uint16 nVal) {
  output[1] = nVal & 0xff;
  output[0] = nVal >> 8;
  return output+2;
}
char *put_be24(char *output,uint32 nVal) {
  output[2] = nVal & 0xff;
  output[1] = nVal >> 8;
  output[0] = nVal >> 16;
  return output+3;
}
char *put_be32(char *output, uint32 nVal) {
  output[3] = nVal & 0xff;
  output[2] = nVal >> 8;
  output[1] = nVal >> 16;
  output[0] = nVal >> 24;
...
char * put_be64(char *output, uint64 nVal) {
