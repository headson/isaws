...
bool CTcpClient::isSendAllBuffer() {
  return ((b_send_all_buffer_ == 1) ? true : false);
}

bool CTcpClient::isRecvOnePacket() {
  return ((b_recv_one_packet_ == 1) ? true : false);
}
